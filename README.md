...
经过我几年的项目实践，HandyJSON库是真的香，JSON转模型，方便。 ```但是有一点不得不提一下```,就是HandyJSON ```稳定性```相关的一些问题❗️❗️❗️，Swift5.0 的时候出过一个泛型解析失败的bug,后来修好了，iOS13.4 beta 的时候由于Swift改动底层源码 导致HandyJSON崩溃。 因为这个问题我们公司的项目专门发了一个bug fixed版本。   从稳定性的角度可以用业界比较多的SwiftJSON + Codable 来做JSON转模型。 
