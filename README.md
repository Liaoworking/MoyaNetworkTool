...
-------2020.03.07 update----
经过我几年的项目实践，HandyJSON库是真的香，JSON转模型，特别方便。 ```但是有一点不得不提一下```,就是稳定性，Swift5.0 的时候出过一个泛型解析失败的bug fixed     iOS13.4 beta 的时候由于Swift改动底层源码 导致崩溃。  因为这个问题专门发了一个bug fixed版本。   从稳定性的角度可以用业界比较多的SwiftJSON + Codable 来做JSON转模型。 
